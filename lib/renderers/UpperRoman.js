...
module.exports = class UpperRoman {
  ...
  render(counterValue) {
    ...
    if (counterValue >= 3e5) {
      return `ↈↈↈ${
        counterValue === 3e5 ? '' : this.render(counterValue % 1e5)
      }`;
    }
    if (counterValue >= 2e5) {
      return `ↈↈ${counterValue === 2e5 ? '' : this.render(counterValue % 1e5)}`;
    }
    if (counterValue >= 1e5) {
      return `ↈ${counterValue === 1e5 ? '' : this.render(counterValue % 1e5)}`;
    }
    if (counterValue >= 9e4) {
      return `ↂↈ${counterValue === 9e4 ? '' : this.render(counterValue % 1e4)}`;
    }
    if (counterValue >= 8e4) {
      return `ↇↂↂↂ${
        counterValue === 8e4 ? '' : this.render(counterValue % 1e4)
      }`;
    }
    if (counterValue >= 7e4) {
      return `ↇↂↂ${
        counterValue === 7e4 ? '' : this.render(counterValue % 1e4)
      }`;
    }
    if (counterValue >= 6e4) {
      return `ↇↂ${counterValue === 6e4 ? '' : this.render(counterValue % 1e4)}`;
    }
    if (counterValue >= 5e4) {
      return `ↇ${counterValue === 5e4 ? '' : this.render(counterValue % 1e4)}`;
    }
    if (counterValue >= 4e4) {
      return `ↂↇ${counterValue === 4e4 ? '' : this.render(counterValue % 1e4)}`;
    }
    if (counterValue >= 3e4) {
      return `ↂↂↂ${
        counterValue === 3e4 ? '' : this.render(counterValue % 1e4)
      }`;
    }
    if (counterValue >= 2e4) {
      return `ↂↂ${counterValue === 2e4 ? '' : this.render(counterValue % 1e4)}`;
    }
    if (counterValue >= 1e4) {
      return `ↂ${counterValue === 1e4 ? '' : this.render(counterValue % 1e4)}`;
    }
    if (counterValue >= 9e3) {
      return `Mↂ${counterValue === 9e3 ? '' : this.render(counterValue % 1e3)}`;
    }
    if (counterValue >= 8e3) {
      return `ↁMMM${
        counterValue === 8e3 ? '' : this.render(counterValue % 1e3)
      }`;
    }
    if (counterValue >= 7e3) {
      return `ↁMM${
        counterValue === 7e3 ? '' : this.render(counterValue % 1e3)
      }`;
    }
    if (counterValue >= 6e3) {
      return `ↁM${counterValue === 6e3 ? '' : this.render(counterValue % 1e3)}`;
    }
    if (counterValue >= 5e3) {
      return `ↁ${counterValue === 5e3 ? '' : this.render(counterValue % 1e3)}`;
    }
    if (counterValue >= 4e3) {
      return `Mↁ${counterValue === 4e3 ? '' : this.render(counterValue % 1e3)}`;
    }
    if (counterValue >= 3e3) {
      return `MMM${
